...
class String
  def caesar(shift)
    alpha=("a".."z").to_a
    altered=alpha[shift..-1]+alpha[0...shift]
    cipher_str=""
    self.each_char do |ch|
      cipher_str+= altered[alpha.index(ch.downcase)]
    end
    cipher_str
  end
end
...
class Hash
  def difference(other_hash)
    new_hash=Hash.new
    self.each do |k,v|
      new_hash[k]=v if other_hash.key?(k)==false
    end
    other_hash.each do |k,v|
      new_hash[k]=v if self.key?(k)==false
    end
    new_hash
  end
end
...
class Fixnum
  def stringify(base)
    ...
    while leftover > 0
      ...
      num_str = digits[low_digit]+num_str
