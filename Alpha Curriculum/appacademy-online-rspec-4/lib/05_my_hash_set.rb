...
class MyHashSet
  ...
  def delete(el)
    item_deleted=store.delete(el)
    item_deleted.nil? ? false : true
  end
  ...
  def union(other_set)
    ...
    other_set.to_a.each {|key|new_set.insert(key)}
    new_set
  end

  def intersect(other_set)
    new_set=self.class.new
    self.to_a.each {|key|new_set.insert(key) if other_set.include?(key)}
    other_set.to_a.each {|key|new_set.insert(key) if self.include?(key)}
    new_set
  end

  def minus(other_set)
    new_set=self.class.new
    self.to_a.each {|key|new_set.insert(key) if other_set.include?(key)==false}
    new_set
  end
